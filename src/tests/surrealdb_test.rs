...
mod tests {
    ...
    async fn test_graph() -> Result<(), Box<dyn std::error::Error>> {
        ...
        // insert
        let alice: Record = db.create("person")
            .content(PersonV2 { name: "Alice", age: 30 })
            .await?.unwrap();

        let bob: Record = db.create("person")
            .content(PersonV2 { name: "Bob", age: 25 })
            .await?.unwrap();
        //
        let org: Record = db.create("organization")
            .content(Organization { name: "SurrealDB Inc." })
            .await?.unwrap();

        // 建立关系：Alice 属于 该组织
        // 使用 RELATE语句创建边，表名为 member_of
        let r: Option<Record> = db.query("RELATE $person->member_of->$org")
    ...
    ")
        ...
        // 更新Bob的年龄为26岁
        let new_bob: Record = db.update(&bob.id)
            .content(PersonV2 { name: "Bob", age: 26 })
            .await?.unwrap();
        dbg!(&new_bob);
        ...
        // 创建新组织 "New Co."
        let new_org: Record = db.create("organization")
            .content(Organization { name: "New Co." })
            .await?.unwrap();
        dbg!("新创建的组织:", &new_org);

        // 创建 Bob 到 "New Co." 的关系
        let bob_to_new_org: Option<Record> = db.query("RELATE $person->member_of->$org")
            .bind(("person", new_bob.id.clone()))
            .bind(("org", new_org.id))
            .await?.take(0)?;
    ...
    async fn test_vector_search() -> Result<(), Box<dyn std::error::Error>> {
        ...
        // 插入带有向量嵌入的文档数据
        let doc1: DocumentRecord = db.create("document")
            .content(Document {
        ...
        let doc2: DocumentRecord = db.create("document")
            .content(Document {
        ...
        let doc3: DocumentRecord = db.create("document")
            .content(Document {
        ...
        let doc4: DocumentRecord = db.create("document")
            .content(Document {
